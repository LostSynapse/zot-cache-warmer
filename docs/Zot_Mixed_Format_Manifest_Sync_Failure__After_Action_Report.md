...
**Zot Server-Side Errors (from `kubectl logs -n zot deployment/zot`):**
...
**Critical Finding:** This identical error occurred **both before and after** adding `"compat": ["docker2s2"]` to the Zot configuration. The docker2s2 compatibility mode did not resolve the issue because the problem was not Docker v2 manifest support (which docker2s2 addresses), but rather the mixed-format structure (OCI index wrapping Docker manifests) which violates spec expectations.

**Complete Sync Failure Pattern:**
The logs showed successful layer downloads for all architectures (amd64, arm64, arm/v7), followed by sync reaching "pushing synced local image to local registry" state, then failure at manifest commit stage in `destination.go:223`. The error persisted across multiple retry attempts.

**Podman Client-Side Behavior:**

Podman successfully attempted to use the Zot mirror but fell back to upstream when sync failed:

```
time="2026-04-21T23:43:44-04:00" level=debug msg="Trying to access \"zot.lost-synapse.com/ghcr-images/catthehacker/ubuntu:act-latest\""
time="2026-04-21T23:43:44-04:00" level=debug msg="GET https://zot.lost-synapse.com/v2/ghcr-images/catthehacker/ubuntu/manifests/act-latest"
```

The pull completed successfully from upstream (ghcr.io) after Zot cache miss, confirming network connectivity and mirror configuration were correct - the issue was specifically with Zot's sync validation logic rejecting the manifest structure.

**Error Code Source:**

From Zot's error definitions (`errors` package):
```go
ErrBadManifest = errors.New("invalid manifest content")
```

This generic error message provided no indication of the actual validation failure (mixed manifest format), significantly complicating diagnosis.
